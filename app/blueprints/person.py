...
def popular():
    ...
    return render_template(
        "person/popular.html", title="Liste d'artistes", persons=persons
    )
...
def details(person_id: int):
    ...
    person["combined_credits"]["cast"] = sorted(
        person["combined_credits"]["cast"], key=get_sort_date, reverse=True
    )

    person["combined_credits"]["crew"] = sorted(
        person["combined_credits"]["crew"], key=get_sort_date, reverse=True
    )
    return render_template(
        "person/details.html", title="Détails d'un acteur", person=person
    )
