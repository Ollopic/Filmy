...
@collection_bp.route("/")
def hub():
    return render_template("collection/index.html", title="Liste de vos collections", collections=client.get_all_collections(request.cookies["token"]))
...
def view(collection_id):
    ...
    return render_template("collection/view.html", title="Vue d'une collection", collection=collection)

@collection_bp.route("/<int:collection_id>/item/<int:movie_id>", methods=["GET"])
def item_details(collection_id: int, movie_id: int):
    return render_template(
        "collection/item/details.html",
...
def add_collection_item():
    ...
    if request.cookies["token"]:
        res = client.add_item_into_collection(request.cookies["token"], collection, film_id, state)
        if res.get("error"):
...
def edit(collection_id):
    if request.method == "POST":
        ...
        if collection_data:
            res = client.update_collection(request.cookies["token"], collection_id, collection_data)
            if res.get("error"):
                flask.flash(res["error"], "error")
    ...
    collection = client.get_collection(request.cookies["token"], collection_id)
    return render_template("collection/edit.html", title="Edition d'une collection", collection=collection)
...
def update_item(collection_id: int, movie_id: int):
    ...
    if token:
        ...
        if res.get("error"):
            flask.flash(res["error"], "error")
            return redirect(url_for("movie.details", collection_id=collection_id, movie_id=movie_id))
